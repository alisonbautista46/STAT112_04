...
ggmap(Twin_Cities) +
  ...
  annotate('text', x=-93.17, y=44.945, label = 'Macalester College', color = I('red'), size = 1.75) +
  theme_map() +
...
favorite_places_alison <- tibble(
  place = c("Saint Paul Home", "Trader Joe's", 
            ...
            "Starbucks", "Minnehaha Falls", "Como Conservatory",  "Chicago Home",  "La Michocana-Pilsen_fav", "Garfield Conservatory", "Lake Michigan Pennisula_fav" ),
  long = c (-93.1676242,  -93.1480801, -93.2025663,  -93.1690109, -93.1900947, -93.2170517, -93.1536019, -87.7230868, -87.6636352,-87.7194441, -87.5816686),
  lat = c(44.9320812, 44.926697, 44.9408164, 44.9330547, 44.917551, 44.9142661, 44.9823653, 41.6973502, 41.856589, 41.8862729, 41.7930772)) %>%
...
covid_19_dates <- covid19 %>%
  filter(date %in% c(as.Date("2020-02-24"), as.Date("2020-05-24"), as.Date("2020-07-24"), as.Date("2020-09-24"))) %>%
  filter(!`state` %in% c("alaska", "hawaii", "guam", "virgin islands", "puerto rico", "northern mariana islands")) %>%
  mutate(state = str_to_lower(state)) %>%
  left_join(census_pop_est_2018) %>%
  mutate(covid_per_10000 = (cases/est_pop_2018)*10000)
...
leaflet(MplsStops, options = leafletOptions(preferCanvas = TRUE)) %>%
  addProviderTiles(providers$OpenStreetMap, options = providerTileOptions(updateWhenZooming = FALSE, updateWhileIdle = TRUE)) %>%
  addCircleMarkers(stroke = FALSE, radius = 2, color = ~pal(problem)) %>%
  addLegend(position = "bottomright",
            pal = pal, 
            values = ~problem,
...
[here](https://github.com/alisonlange45/STAT112_04_exercises)
